...
const Page = () => {
  ...
  const { register, handleSubmit, watch, formState: { errors } } = useForm<updateType>();
  const onSubmit = async (data: updateType) => {
    data.date = new Date(data.date).getTime();
    console.log(data);
    try {
        toast.loading("Loading...");
        await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/updates`, data)
    ...
    <div className="w-[90%] flex gap-x-16 justify-center">
        ...
        <div className="w-1/3 mt-10">
            ...
            <form className="flex flex-col" onSubmit={handleSubmit(onSubmit)}>
                ...
                <Input className="w-full" {...register("link", { required: false })} /> */}

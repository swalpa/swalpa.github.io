                  ...
                  <div key={index} className="w-full flex flex-col items-center">
                    ...
                    <div className="w-[95%] mb-5">
                      {publicationWithYear.publications.filter(obj => obj.category === PublicationCategory.Journal).map((publication, index) => (
                        <div id={publication._id} key={index} className={cn('w-full flex flex-col lg:flex-row items-start gap-x-8 p-1 lg:p-2 text-justify my-1', index !== 0 && 'border-t border-black/10')}>
                          ...
                          <div className="w-[95%] lg:text-xl flex flex-col gap-2 font-medium">
                            {publication.paperLink ? <div className="flex gap-x-2"><Link target="_blank" className="text-blue-500 hover:underline cursor-pointer" href={publication.paperLink}>{publication.title}</Link> {publication.highlighted && <section className="hidden lg:block animate-pulse text-red-600 text-xs min-w-[170px]">ESI Highly Cited Paper</section>}</div> :
                              <div className="flex gap-x-2"><p className="font-medium">{publication.title}</p>{publication.highlighted && <section className="hidden lg:block animate-pulse text-red-600 text-xs min-w-[170px]">ESI Highly Cited Paper</section>}
                              </div>}
                      ...
                      {publicationWithYear.publications.filter(obj => obj.category === PublicationCategory.Conference).map((publication, index) => (
                        <div id={publication._id} key={index} className={cn('w-full flex flex-col lg:flex-row items-start gap-x-8 p-1 lg:p-2 text-justify border-t border-black/10')}>
                          <div className="w-[160px] lg:w-[2%] pt-1 my-2 lg:my-0 flex gap-x-2 justify-between items-center">
                            <p className="font-semibold">{publication.serialNumber}</p>
                            {publication.highlighted && <section className="animate-pulse text-red-600 text-xs block lg:hidden min-w-[170px]">ESI Highly Cited Paper</section>}
                          </div>
